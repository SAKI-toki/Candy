...
namespace impl
{
	...
	void RootSignatureStartupInfoImpl::setDescriptorRange(const s32 _rootParameterIndex, const types::ShaderRegisterInfo _shaderRegisterInfo,
	...
	void RootSignatureStartupInfoImpl::setStaticSampler(const s32 _index, const types::ShaderRegisterInfo _shaderRegisterInfo,
	...
	void RootSignatureStartupInfoImpl::onRootSignatureFlag(const types::ROOT_SIGNATURE_FLAG _rootSignatureFlag)
	{
		m_StartupInfo.Flags |= ConvRootSignatureFlag(_rootSignatureFlag);
	}

	void RootSignatureStartupInfoImpl::offRootSignatureFlag(const types::ROOT_SIGNATURE_FLAG _rootSignatureFlag)
	{
		m_StartupInfo.Flags &= ~ConvRootSignatureFlag(_rootSignatureFlag);
	}
}
