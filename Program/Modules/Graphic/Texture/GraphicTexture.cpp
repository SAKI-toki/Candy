...
{
	m_CommandList.startup(GraphicManager::GetDevice(), types::COMMAND_LIST_TYPE::RENDERING, Config::GetBackBufferCount());
	m_CommandList.close();
}
...
void TextureManager::ExecuteUploadTexture(const CommandQueue& _commandQueue)
{
	...
	m_CommandList.preDraw(GraphicManager::GetBackBufferIndex());
	for (auto& uploadBufferInfo : m_UploadBufferInfos)
	{
		m_CommandList.copyTexture(GraphicManager::GetDevice(), uploadBufferInfo.m_DstBuffer, uploadBufferInfo.m_SrcBuffer);
	}
	m_UploadBufferInfos.clear();
	m_CommandList.close();
...
void TextureManager::CreateTexture(Buffer& _buffer, const std::byte* const _pixels, const u64 _size)
{
	...
	uploaderBuffer.startup(GraphicManager::GetDevice(), uploaderBufferStartupInfo);
